...
export default function UploadingFileProgress({
  driveFileForm,
}: {
  driveFileForm: DriveFileModelForm;
}) {
  ...
  const updateProgressPercentage = (progressPerecentageValue: number) => {
    if (
      progressPerecentageValue >= 99 ||
      driveFileForm.uploadingStatus === UploadingStatus.ERROR
    )
      return;
    ...
    setTimeout(
      () => updateProgressPercentage(progressPerecentageValue + 1),
      100 * (progressPerecentageValue / 20),
    );
  };
  ...
  return (
    <>
      <Text className={styles[driveFileForm.uploadingStatus]}>
        {driveFileForm.name}
      </Text>
      <Progress
        status={status}
        percent={progressPerecentage}
