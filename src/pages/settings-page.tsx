...
import { AndroidOutlined, AppleFilled, DeleteOutlined, DesktopOutlined, MobileOutlined, QuestionOutlined, WindowsOutlined } from "@ant-design/icons";
...
export default function SettingsPage() {
  ...
  const onDeleteSession = (userSession: UserSessionModel) => {
    UsersService
      ...
      .then(() => {
        setUserSessions((state) => state.filter((element) => element.id !== userSession.id));
  ...
  const sessionToTableItem = (userSession: UserSessionModel) => {
    const {id, clientIp, operatingSystem, userAgent, deviceType, createdAt, updatedAt, lastUsedAt} = userSession;
        ...
        <Collapse>
          <Collapse.Panel header="User sessions" key="user-sessions-1">
            <Table className={tableStyles.table} dataSource={tableDataSource}>
              <Column key='session-device' title='Device' dataIndex='device' render={(_, record: UserSessionModel) => (
                <Space>
              ...
              <Column key='session-userAgent' title='User Agent' dataIndex='userAgent' />
              <Column key='session-createdAt' title='Created At' dataIndex='createdAt' />
              <Column key='session-lastUsedAt' title='Last used at' dataIndex='lastUsedAt' />
              <Column key='session-actions' title='Actions' render={(_, record: UserSessionModel) => (
                <Space>
                  <Tooltip title="Delete session">
                    <Button onClick={() => onDeleteSession(record)} danger shape="circle" icon={<DeleteOutlined />} />
                  </Tooltip>
                </Space>
              )} />
            </Table>
