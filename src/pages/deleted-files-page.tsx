...
import { Alert, Button, Col, Row, TablePaginationConfig, Tag, Tooltip } from "antd";
...
export default function DeletedFilesPage() {
  ...
  const onRestore = (restoredFile: DriveFileModel) => {
    if (restoredFile.id) {
      DriveFilesService.restore(restoredFile.id).then(() => {
        setFiles((state) => {
...
function FileRow({file, onRestore}: {file: DriveFileModel, onRestore: () => void;}) {
  return {
    key: file.id,
